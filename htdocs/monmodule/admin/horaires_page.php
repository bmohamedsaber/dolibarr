...
/**
 *   	\file       dev/Horairess/Horaires_page.php
...
dol_include_once('/monmodule/class/horaires.class.php');
...
if ($action == 'add')
{
	...
	{
		$error++;
		setEventMessage($langs->trans("ErrorFieldRequired",$langs->transnoentitiesnoconv("bonjour")),'errors');
	}
	...
	if (! $error)
	{
		...
		{
			// Creation OK
			$urltogo=$backtopage?$backtopage:dol_buildpath('/monmodule/',1);
			header("Location: ".$urltogo);
			exit;
		}
		{
...
if ($action == 'list' || empty($id))
{
    ...
    $sql.= " FROM ".MAIN_DB_PREFIX."horaires as t";
   // $sql.= " WHERE  = 'xxx'";
